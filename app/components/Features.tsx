        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ textAlign: "center", maxWidth: 680, margin: "0 auto 64px" }}>
            ...
            <h2 style={{ fontFamily: "Epilogue,sans-serif", fontWeight: 900, fontSize: "clamp(28px, 4vw, 48px)", color: tc, margin: "0 0 16px", letterSpacing: "-0.03em", lineHeight: 1.1 }}>
              Everything your organisation needs to<br /><span style={{ color: "var(--teal)" }}>engage, measure, and thrive.</span>
            </h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 18 }}>
            {features.map((f, i) => (
              <div key={i} className="benefit-card">
                ...
                <h3 style={{ fontFamily: "Epilogue,sans-serif", fontWeight: 700, fontSize: 16, color: tc, margin: "0 0 8px" }}>{f.title}</h3>
      ...
      <section style={{ background: altBg, padding: "80px 0" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            ...
            <h2 style={{ fontFamily: "Epilogue,sans-serif", fontWeight: 900, fontSize: "clamp(24px, 3.5vw, 42px)", color: tc, margin: 0, letterSpacing: "-0.025em" }}>
              The numbers behind the urgency.
            </h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 24 }}>
            {marketStats.map((s) => (
              <div key={s.label} style={{ textAlign: "center", background: isDark ? "var(--navy-3)" : "white", border, borderRadius: 20, padding: "36px 24px" }}>
                <div style={{ fontSize: 36, marginBottom: 12 }}>{s.icon}</div>
                <div style={{ fontSize: 44, fontWeight: 900, fontFamily: "Epilogue,sans-serif", color: "var(--teal)", lineHeight: 1, marginBottom: 12 }}>{s.value}</div>
